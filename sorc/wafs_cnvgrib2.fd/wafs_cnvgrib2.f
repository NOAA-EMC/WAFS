      program wafs_cnvgrib2
C$$$  MAIN PROGRAM DOCUMENTATION BLOCK
C  
C MAIN PROGRAM:  wafs_cnvgrib2
...
C                      and help messages
C 2011-12-02  Vuong    Changed Master Table Version Number from 2 to 6.
...
C     UNIQUE:    - cnvwafs12, cnvwafs21, cnvwafs22, usage
C     LIBRARY:
C       W3LIB    - errexit
...
C  GET ARGUMENTS
      ...
      IF(NARG.lt.3) THEN       ! may be a problem with args
         IF(NARG.eq.0) THEN
            !CALL ERRMSG('wafs_cnvgrib2:  Incorrect usage')
            call usage(0)
            CALL ERREXIT(2)
     ...
     &      table_ver .gt. mastertab ) then
            call usage(0)
            call errmsg ('  ')
            call errmsg('wafs_cnvgrib2: cannot change to master '// 
     &                   'table version ' // master_table_ver)
            call errmsg ('  ')
     ...
     &                  curmastertab_ver)
         ...
         if ( (ipack.ne.-1).and.(outver.eq.1) ) then
            CALL ERRMSG('wafs_cnvgrib2: -pxx option ignored when '//
     &                   'using -g21')
         endif
         !
         !   Must have -g option
         ...
         if ( (inver.eq.0).or.(outver.eq.0) ) then
            CALL ERRMSG('wafs_cnvgrib2: must use one -gxx option')
            call usage(0)
            CALL ERREXIT(2)
         endif
         !
     ...
     &                       ipack.ne.32) ) then
            CALL ERRMSG('wafs_cnvgrib2: -m or -m0 option ignored '//
     &                 'when not using -p2, -p31 or -p32.')
            usemiss=.false.
         endif
      ENDIF
!
...
!
      ...
      if (IOS.NE.0) then
         call errmsg('wafs_cnvgrib2: cannot open input GRIB file '//
     &               gfilein(1:NCGB))
         call errexit(3)
      endif
      ...
      if (IOS.NE.0) then
         call errmsg('wafs_cnvgrib2: cannot open output GRIB file '//
     &               gfileout(1:NCGB))
         call errexit(4)
      endif
!
...
!
      if ((inver.eq.1).AND.(outver.eq.2)) then
         call cnvwafs12(ifl1,ifl2,ipack,usemiss,imiss,uvvect, table_ver)
      elseif ((inver.eq.2).AND.(outver.eq.1)) then
         call cnvwafs21(ifl1,ifl2)
      elseif ((inver.eq.2).AND.(outver.eq.2)) then
         call cnvwafs22(ifl1,ifl2,ipack,usemiss,imiss,table_ver)
      else
...
C
C$$$
         character(len=20) :: wafs_cnvgrib2_ver="wafs_cnvgrib2-1.2.2"
         integer,intent(in) :: iopt 

         if ( iopt.eq.0 ) then
         call errmsg ('  ')
         call errmsg('Usage: wafs_cnvgrib2 [-h] {-g12|-g21|-g22} '//
     &        ' [-m|-m0] [-nv] [-mastertable_ver_x]')
         call errmsg('               [{-p0|-p2|-p31|-p32|-p40'//
     &               '|-p41}]  ingribfile   outgribfile')
         call errmsg ('  ')
         call errmsg('Usage: wafs_cnvgrib2  -h  For helps and '//
     &                                  'show all options') 
         call errmsg ('  ')
         endif
         ...
         if ( iopt.eq.1 ) then
            call errmsg ('  ')
            call errmsg('wafs_cnvgrib2:  version '//wafs_cnvgrib2_ver)
            call errmsg ('  ')
