...
! *
  ...
  type :: m_Radar_t
     ...
     integer :: minVipPercent
     integer :: minDBzPercent
  end type m_Radar_t
  ...
  subroutine m_assignPreviousMemberValue(sectionName, &
    ...
    case("model")		!=====  model  =====!
       ...
       case("name")
          ...
          case("RAP", "NAM", "GFS", "FV3")
             cfg%model%name = trim(ADJUSTL(rawValues(1)))
          case default
             write(*, *) "Config::_assignPreviousMemberValue -- only for RAP/NAM/GFS/FV3 "
             iret = 1
             return
          end select
       case("grib")
    ...
    case("Pressure2Flight")	!=====  Pressure2Flight  =====!
       ...
       case("outputFLT")
          call m_setValue(rawValues(1), cfg%p2f%outputFLT, iret)
       case default
          write(*, *) "Config::_assignPreviousMemberValue -- Section ", sectionName, " doesn't include member: ", memberName
          iret = 1
          return
    ...
    case("RadarProcessing")	!=====  Radar:Processing  =====!
       ...
       case("minVipPercent")
          call m_setValue(rawValues(1), cfg%radar%minVipPercent, iret)
       case("minDBzPercent")
          call m_setValue(rawValues(1), cfg%radar%minDBzPercent, iret)
       case default
          write(*, *) "Config::_assignPreviousMemberValue -- Section ", sectionName, " doesn't include member: ", memberName
          iret = 1
          return
...
! *
  ...
  subroutine printConfig(cfg)
    ...
    write(*,*) cfg%radar%minVipPercent
    write(*,*) cfg%radar%minDBzPercent
