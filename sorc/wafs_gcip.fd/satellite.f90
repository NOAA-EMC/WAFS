...
!open(71,file="innersatch4.sample",form="unformatted", access="direct", recl=nx*ny*4)
!write(71, rec=1, iostat=iret) satData%ch4
!if(iret /= 0) write(*,*) "wrong inner sat",iret
!close(71)
...
!open(71,file="innersatch4ss.sample",form="unformatted", access="direct", recl=nx*ny*4)
!write(71, rec=1, iostat=iret) satData%ch4
!if(iret /= 0) write(*,*) "wrong inner sat",iret
!close(71)
  ...
  subroutine decodeMcIDAS(filename, kgds, ibrightness, iret)
    ...
    case('MERC')
       ...
       write(*,*) "Satellite GDS of", filename, kgds(1:20)
  ...
  SUBROUTINE m_calcGridValues(kgds, p, e, n)
    ...
    ! variables for GDSWZD
    real, allocatable :: lat(:,:), lon(:,:), x(:,:), y(:,:), crot(:,:), srot(:,:)
    ...
    npts = nx * ny
    call GDSWZD(kgds,iopt,npts,fill,x,y,lon,lat,nret,crot,srot)
    deallocate(x)
    deallocate(y)
    deallocate(crot)
    deallocate(srot)
