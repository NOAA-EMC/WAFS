...
!> GFS has different ice condensate and liquid condensate from RAP NAM FV3
!------------------------------------------------------------------------------
  ...
  subroutine calculateDerived_Fields(nx, ny, nz, cmodel, fields, modelData, iret)
    ...
    character(*), intent(in) :: cmodel ! RAP NAM GFS FV3
    ...
    if(cmodel == "GFS") then
       ! use the same approach as in nwprod/global_shared.v14.1.0/sorc/global_fcst.fd/GSM/gsmphys/radiation_clouds.f
...
!------------------------------------------------------------------------------
  ...
  elemental real function m_calcLiftedCondLevelTemp(temperature, dewPointTemp)
    ...
    if(abs(dewPointTemp-56.0) < EPSILON) return
...
!------------------------------------------------------------------------------
  ...
  elemental real function m_calcSuperCooledLiquidWater(&
    ...
    ! do not convert from kg/kg to g/kg. Otherwise too much noise according to
    ! icing.f90::icing_repmapN(), line: if (LWC > 0.0000001) SLW = 1.0
    if(temperature <= FREEZING) m_calcSuperCooledLiquidWater = cloudWaterMixingRatio + rainWaterMixingRatio
...
!------------------------------------------------------------------------------
! Description
!> Same as the approach in in nwprod/global_shared.v14.1.0/sorc/global_fcst.fd/GSM/gsmphys/radiation_clouds.f
!
!> @param[in] temperature             - in degrees K
!> @param[in] cloudWaterMixingRatio   - in kg/kg
...
!> @param[in] pressure        - in Pascals
!> @param[in] temperature     - in degrees K
!> @param[in] liqCondensate   - in g/kg
!> @param[in] iceCondensate   - in g/kg
!> @param[out] totalWaterPath - g/m2
!------------------------------------------------------------------------------

   subroutine m_calcTotalWaterPath(height, pressure, temperature, &
                                   iceCondensate, liqCondensate, totalWaterPath)
    real, dimension(:, :, :), intent(in) :: height, pressure, temperature, &
                                         iceCondensate, liqCondensate
    real, dimension(:, :, :), intent(out) :: totalWaterPath

    real :: totalCondensate, integratedCond
    ...
    do i = 1, m_nx
      ...
      do k = m_nz, 1, -1
         ...
         do m = k, m_nz - 1
            ...
            if((iceCondensate(i,j,m) < MISSING + 1.) .or. &
               (liqCondensate(i,j,m) < MISSING + 1.)) cycle
            totalCondensate = iceCondensate(i,j,m) + liqCondensate(i,j,m) 

            if (totalCondensate > 0.001) then
               topIdx =m
            else
               exit
            end if
         ...
         do m = topIdx, baseIdx, -1
            if(temperature(i,j,m) <= FREEZING) then
               ...
               integratedCond = integratedCond + ((totalCondensate * pressure(i, j, m))/ &
                                (GAS_CONSTANT * temperature(i, j, m))) * &
                                (height(i, j, m+1) - height(i, j, m) + 0.001) 
            end if
