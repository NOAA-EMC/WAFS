...
!----------------------------------------------------------------------------
  subroutine readModelGB2(filename, cmodel, pds7, fields, modelData, gfld, kgds, iret)
    ...
    character(len=*), intent(in) :: cmodel ! RAP/NAM/GFS/FV3
    ...
    case("RAP")
       ...
       pdt%snm	= m_pdt_t(1, 25)
       pdt%gpm	= m_pdt_t(1, 32)
       pdt%icm	= m_pdt_t(6, 0) ! the same as ICMR, just conventional difference for RAP
       pdt%rh	= m_pdt_t(-1, -1)
    ...
    do k = 1, nz
       ...
       if(pdt%gpm%icat >= 0) then
          ...
          if(iret /= 0)  go to 10
          where(fields%gpm > SPVAL -1) fields%gpm = MISSING
       end if
  ...
  subroutine getHeaderGB2(iunit, kgds, igrid, iret, gfld_out)
    ...
    do
       ...
       if (iseek < 0) then
          ...
          if ( present(gfld_out)) then
             gfld_out = gfld
             ! Hold the memory of gfld for writing, call gt_free after writing
          else
             call gf_free(gfld)
          end if
          exit
...
!----------------------------------------------------------------------------
  subroutine m_pdt2gds(gfld, kgds, igrid)
    ...
    ! prepare for gdt2gds() to get kgds used by gdswzd()
    integer :: igds(5)
    integer :: ideflist(1)
    integer :: iret
