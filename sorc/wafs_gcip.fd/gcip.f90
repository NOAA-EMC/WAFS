...
!**********************************************************************
  ...
  integer :: nx, ny, nz, nfiner, iret,i,j,k
...
real, allocatable :: realData3D(:,:, :) ! used for output data into binary file
  ...
  print *, "RODUCT TEMPLATE 4=", (gfld%ipdtmpl(j),j=1,gfld%ipdtlen)
  ...
  elseif(igrib == 2) then
     ...
     gfld%ipdtmpl(9) = 0
     print *, "ipdtmpl=", gfld%ipdtmpl(1:15)
  end if
...
!call writeBIN3D(nx, ny, 5, "slw.sample", inputs%model%slw(:,:,20:24))
...
!endif
  ...
  if(len_trim(pirepFile) /= 0) then
     call runPirep(trim(pirepFile), iruntime, cfg, kgds, inputs%model%h, inputs%pirep, iret)

!if(iret == 0) then
!allocate(realData3D(nx, ny, 8))
!do k=1,8
!do j=1,ny
!do i=1,nx
!   realData3D(i,j,k) =  inputs%pirep(i,j,12+k)%interest
!end do
!end do
!end do
!call writeBIN3D(nx, ny, 8, "pirepInterest.sample", realData3D)
!do k=1,8
!do j=1,ny
!do i=1,nx
!   realData3D(i,j,k) =  inputs%pirep(i,j,12+k)%weight
!end do
!end do
!end do
!call writeBIN3D(nx, ny, 8, "pirepWeight.sample", realData3D)
...
!end if
  ...
  if(len_trim(lightningFile) /= 0) then
