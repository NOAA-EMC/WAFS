...
contains
  ...
  integer function getPireps(infile, iruntime, timeWindow, pireps)
   ...
   integer :: i, iret
    ...
    ! write(*,*) "start_time, end_time=", start_time, end_time
    ...
    loop_msg: do while(ireadmg(iunit, msgtype, report_date) == 0)
       ...
       ! write(*,*) "msgtype, report_date=", msgtype, report_date
...
! print *, "before matching pirep=", icingNumReports, icing(1,1),  icing(2,1), icing(1,2),  icing(2,2)
...
! print *,  "after matching pirep1=",  pirepPointer%iceBase1, pirepPointer%iceTop1, pirepPointer%iceIntens1
! print *,  "after matching pirep2=",  pirepPointer%iceBase2, pirepPointer%iceTop2, pirepPointer%iceIntens2
    ...
    print *, "PIREPs count(valuable, valid, reports) =", getPireps, count_valid, count_reports
  ...
  subroutine m_mapPirep(lrun_time, nz, heights, pirep, pirepCounts, pirepData)
    ...
    type(pirep_report_t), intent(inout) :: pirep  ! should be in, but restricted by GDSWZD
...
!print *, "Adjust Pirep lat,lon=",pirep%lat, pirep%lon
!print *, "Adjust Pirep=",iceBase
!print *, "Adjust Pirep=",iceTop
!print *, "Adjust Pirep=",iceIntensity
!print *, "Adjust Pirep=",temporalInterest,pirep%obsTime,lrun_time
...
!print *, "nearby pireps=", i,j,dist,radialInterest
...
!print *, k,heights(i,j,k), gap,verticalIterest,weight,interest,pirepData(i, j, k)%weight,pirepData(i, j, k)%interest,pirepCounts(i, j, k)
          end if
