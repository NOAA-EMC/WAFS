...
! values used to write data to GRIB file
type gparms_t
...
type input_data_t
    ...
    real(kind=r_kind), dimension(:,:), allocatable :: sfc_pres, &
        conv_pres_bot, conv_pres_top, conv_pcp_rate, tot_cld_cover, &
        conv_cld_cover
    real(kind=r_kind), dimension(:,:,:), allocatable :: hgt, t, u_wnd, &
        v_wnd, vvel, rh, cld_cover
...
type cfg_t
    ...
    type(fuzzy_set_t) :: icng_t, icng_conv_t, icng_cld_cover, icng_vvel, &
        icng_cld_top_t
    real(kind=r_kind) :: icng_min_cld_cover
