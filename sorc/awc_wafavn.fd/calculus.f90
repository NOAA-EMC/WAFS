...
contains
!----------------------------------------------------------------------------
subroutine calculus_grad(nx, ny, msng, f, dx, dy, df_dx, df_dy)
! calculates gradient of mass variable
    ...
    real(kind=r_kind), dimension(nx,ny), intent(out) :: &
        df_dx, df_dy ! calculated gradient
...
!    df_dy(:,1) = df_dy(:,2)
!    df_dy(:,ny) = df_dy(:,ny-1)
end subroutine calculus_grad
    
!----------------------------------------------------------------------------
subroutine calculus_d_dh(nx, ny, nz, msng, f, h, df_dh)
