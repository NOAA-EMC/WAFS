...
if [[ "${fhr}" == "anl" ]]; then # Analysis
    ...
    VDATE="${PDY}${cyc}"
    ATMINP="${COMINgfs}/analysis/atmos/gfs.t${cyc}z.analysis.atm.a006.nc"
    FLXINP="${COMINgfs}/analysis/atmos/gfs.t${cyc}z.analysis.atm.a006.nc"
    PostFlatFile="${PARMwafs}/upp/postxconfig-NT-gfs-wafs-anl.txt"

else # Forecast

    VDATE=$(${NDATE} +${fhr} ${PDY}${cyc})
    ATMINP="${COMINgfs}/model/atmos/history/gfs.t${cyc}z.atm.f${fhr}.nc"
    FLXINP="${COMINgfs}/model/atmos/history/gfs.t${cyc}z.sfc.f${fhr}.nc"
    ifhr="$((10#${fhr}))"
    if ((ifhr <= 48)); then
