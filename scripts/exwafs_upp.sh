...
POSTGPSH=${POSTGPSH:-$USHgfs/wafs_nceppost.sh}
PREFIX=${PREFIX:-${RUN}.t${cyc}z.}
...
export stime=`echo $fhr | cut -c1-3`
############################################################
...
#----------------------------------
   ...
   if [ $OUTTYP -eq 4 ] ; then
      export loganl=$COMINgfs/${PREFIX}atmanl${SUFFIX}
   else
      export loganl=$COMINgfs/${PREFIX}sanl
   fi
